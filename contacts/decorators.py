...
def classmethod_cache(seconds=0):
    """
    A `seconds` value of `0` means that we will not memcache it.

    If a result is cached on class, return that first.  If that fails, check
    memcached. If all else fails, hit the db and cache on class and in
     memcache.
